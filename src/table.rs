...
impl Table {
    ...
    /// Filters the table based on the provided nested condition structure and displays the records that satisfy the conditions.
    ///
    /// # Arguments
    ///
    /// * `nested_condition` - A `NestedCondition` enum representing the nested condition structure.
    ...
    /// table.insert(vec!["3".to_string(), "Charlie".to_string(), "19".to_string()]).unwrap();
    ///
    /// // Filter records where "age" is 30
    /// let nested_condition = NestedCondition::Condition(
    ///     "age".to_string(),
    ///     "=".to_string(),
    ///     "30".to_string(),
    /// );
    /// table.filter(nested_condition);
    ///
    ...
    /// table.filter(nested_condition);
    /// ```
    pub fn filter(&self, nested_condition: NestedCondition) {
        let columns_clone = self.columns.clone();
        let max_column_name_len = self
            .columns
            .iter()
        ...
        // Print the data rows that satisfy the condition
        let max_rows = self
            .columns
            .iter()
            .map(|column| column.data.len())
        ...
        for row_idx in 0..max_rows {
            let update_record = evaluate_nested_conditions(&nested_condition, &columns_clone, row_idx)
                .unwrap_or(false);
